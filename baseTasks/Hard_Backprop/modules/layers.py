...
class BatchNormalization(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            ...
            grad_input_mean_from_norm = grad_norm * self.inv_sqrt_var
            grad_inv_sqrt = np.sum(grad_norm * self.input_mean, axis=0)
            grad_var = grad_inv_sqrt * (-1/2) * self.inv_sqrt_var ** 3
            grad_input_mean_from_var = grad_var * 2 * self.input_mean / input.shape[0]
            grad_input_mean = grad_input_mean_from_norm + grad_input_mean_from_var
            grad_mean = -np.sum(grad_input_mean, axis=0)
            grad_input_from_mean = grad_mean / input.shape[0]

            return grad_input_mean + grad_input_from_mean
        else:
            if self.affine:
                return grad_output * self.weight * self.inv_sqrt_var
            
            return grad_output * self.inv_sqrt_var
...
class Dropout(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if self.training:
            norm = 1 / (1 - self.p)
            return norm * self.mask * grad_output
        else:
            return grad_output
...
class Sequential(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        for i in range(len(self.modules) - 1, 0, -1):
            output = self.modules[i - 1].output
            grad = self.modules[i].backward(output, grad)
        return self.modules[0].backward(input, grad)
