...
class SGD(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m in zip(parameters, gradients, self.state['m']):
            ...
            np.add(self.momentum * m, grad_with_decay, out=m)
            param -= self.lr * m
...
class Adam(Optimizer):
    ...
    def step(self):
        ...
        for param, grad, m, v in zip(parameters, gradients, self.state['m'], self.state['v']):
            ...
            """
            grad_with_decay = grad + self.weight_decay * param
            np.add(self.beta1 * m, (1 - self.beta1) * grad_with_decay, out=m)
            np.add(self.beta2 * v, (1 - self.beta2) * (grad_with_decay ** 2), out=v)
