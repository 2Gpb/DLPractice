...
class BatchNormalization(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
          ...
          if batch_size > 1:
              unbiased_var = self.var * batch_size / (batch_size - 1)
          else:
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        if self.training:
          sum_grad = np.sum(grad_norm, axis=0)
          sum_grad_norm = np.sum(grad_norm * self.norm_input, axis=0)
          ...
          grad_input = (
              ...
              * (
                  ...
                  - sum_grad
                  - self.norm_input * sum_grad_norm
              )
          )
        else:
          grad_input = grad_norm * self.inv_sqrt_var
...
class Dropout(Module):
    ...
    def compute_output(self, input: np.ndarray) -> np.ndarray:
        ...
        if self.training:
          ...
          return input * self.mask / (1 - self.p) if self.p != 0 else input
        else:
          self.mask = None
          return input
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        """
        if self.training:
          return grad_output * self.mask / (1 - self.p) if self.p != 0 else grad_output
        else:
          return grad_output
...
class Sequential(Module):
    ...
    def compute_grad_input(self, input: np.ndarray, grad_output: np.ndarray) -> np.ndarray:
        ...
        for i in range(len(self.modules) - 1, -1, -1):
          grad_input = self.modules[i].backward(inputs[i], grad_input)
